...
#[cfg(not(feature = "wasm"))]
impl<T: crate::DataService<Error: Send> + Send + Sync + 'static> DataServiceBounds for T {}
...
/// Runs a collection of futures in parallel, collecting their outputs.
///
...
/// via `join_all`.
#[cfg(not(feature = "wasm"))]
pub(crate) async fn join_parallel<T, F>(futures: impl IntoIterator<Item = F>) -> Vec<T>
where
    T: Send + 'static,
